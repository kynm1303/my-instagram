...
import { reload, storeUserPosts } from '../../../redux/actions';
...
function Save(props) {
    ...
    useLayoutEffect(() => {
        props.navigation.setOptions({
            headerRight: () => (
                <Feather style={navbar.image} name="check" size={20} color="green" onPress={() => { uploadImage() }} />
            ),
        });
    }, [caption]);
    ...
    const SaveStorage = async (image, path) => {
        ...
        console.log("SaveStorage", image, path);
    ...
    const handleFetchUserPost = function () {
